...
export function GroupCreate() {
  ...
  return (
    <div className="grid gap-4 animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out xl:grid-cols-[0.8fr_1.2fr]">
              ...
              {friends.length === 0 ? (
                ...
                friends.map((friend) => (
                  <button
                    key={friend.id}
                    type="button"
                    className={`flex min-h-11 w-full cursor-pointer items-center justify-between rounded-xl border p-3 text-left transition-colors ${
                      selectedIds.has(friend.id) ? "border-[var(--accent)] bg-[var(--accent)]/5" : "bg-white/50 hover:bg-white/80"
                    }`}
